...
\begin{tcolorbox}[
    ...
    Contoh:
    Gerakan 1: tiles traversed costs = 2, 5, 4 -> movement cost = 11
    Gerakan 2: tiles traversed costs = 3, 2 -> movement cost = 5
...
\begin{enumerate}
    ...
    \item Playback (bukan live update): setelah algoritma selesai, program harus dapat memvisualisasikan proses pathfinding sebagai rangkaian langkah yang dapat diputar mundur/maju. Untuk CLI cukup memenuhi fitur berikut:
          \begin{enumerate}
...
Catatan: untuk mode GUI (bonus), playback harus memiliki kontrol kecepatan (play/pause/seek) dan kemampuan maju/mundur yang setara dengan pengalaman video.
...
\begin{itemize}
    ...
    \item Petak angka adalah tile normal (bukan rintangan): aktor akan meluncur melewatinya jika bergerak tanpa berhenti.
    \item Jika aktor bergerak menuju sisi papan (kanan/atas/kiri/bawah) dan tidak ada rintangan pada sisi tersebut untuk menghentikannya, maka aktor akan keluar papan dan kondisi dianggap game over; permainan harus diulang dari awal.
...
\begin{enumerate}
    \item \textbf{GUI penuh untuk program (maks 6 poin)}: membuat antarmuka grafis yang memungkinkan pengguna memilih file input, menampilkan papan, mengatur kecepatan playback, dan memutar langkah mundur/maju. Kriteria penilaian:
          \begin{itemize}
              \item 4 poin: GUI menampilkan papan, lintasan akhir, dan kontrol playback (play/pause, step forward/back, seek).
              \item 1 poin: Opsi untuk menyimpan state (iterasi) ke file dari GUI.
              \item 1 poin: Penggunaan UI/UX yang rapi dan dokumentasi singkat cara menjalankan GUI.
          \end{itemize}
    ...
    \item \textbf{Tambahkan algoritma pathfinding tambahan (maks 2 poin)}: implementasi setidaknya satu algoritma pathfinding lain selain yang diwajibkan (mis.\, Dijkstra, IDA*, BFS adaptif untuk sliding puzzle, atau algoritma lain yang relevan). Penilaian:
          \begin{itemize}
              \item 2 poin: Implementasi yang benar, teruji pada beberapa kasus, dan dibandingkan singkat dengan algoritma bawaan (catat perbedaan hasil atau performa).
          \end{itemize}
    ...
    \item \textbf{Tambahkan dua alternatif heuristik (maks 2 poin)}: sediakan dua heuristik tambahan untuk A* selain heuristik utama. Kriteria:
          \begin{itemize}
              \item 1 poin per heuristik: heuristik yang konsisten/beralasan, diimplementasikan, dan diuji pada minimal 2 kasus sehingga menunjukkan perbedaan perilaku pencarian.
          \end{itemize}
    ...
    \item \textbf{Optimasi kecepatan pencarian (maks 5 poin, tetapi total bonus tetap dibatasi)}: optimasi yang dapat meningkatkan kecepatan menemukan solusi. Penilaian didasarkan pada bukti eksperimen dan metrik yang jelas.
          \begin{itemize}
              \item 2 poin: Profiling sederhana dan pengurangan waktu eksekusi (mis.\, perbandingan waktu rata-rata vs implementasi dasar) minimal pada 3 kasus uji.
              \item 2 poin: Perbaikan algoritmis atau struktur data yang signifikan (mis.\, penggunaan bitmasking, pruning, atau struktur heap yang lebih efisien).
              \item 1 poin: Dokumentasi perubahan dan instruksi reproduksi eksperimen (perintah untuk menjalankan benchmark).
          \end{itemize}
\end{enumerate}
